...
def mpoly_mpoly_cfs_bf(mr1, mr2, n=1000):
    cf1, cf2 = poly_poly_v_clip(mr1.at(0), mr2.at(0))
    cfs = [(0, cf1, cf2)]
    for i in range(1, n + 1):
        t = i / n
        _, cf1_prev, cf2_prev = cfs[-1]
        cf1, cf2 = poly_poly_v_clip(mr1.at(t), mr2.at(t), cf1_prev, cf2_prev)
...
def mpoly_mpoly_verify_cfs(mr1, mr2, cfs):
    ...
    for i in range(len(cfs)):
        ...
        cf1_bf, cf2_bf = poly_poly_v_clip(
            mr1.at((ts + te) / 2), mr2.at((ts + te) / 2), cf1, cf2)
...
def mpoly_mpoly_verify_cfs_fast(mr1, mr2, cfs):
    t, cf1, cf2 = cfs[-1]
    cf1_bf, cf2_bf = poly_poly_v_clip(mr1.at(t), mr2.at(t), cf1, cf2)
    ...
    while True:
        ...
        if cf1 % 2 == 0 and cf2 % 2 == 0:
            ...
            if (tmp_cf2_prev == -1
                ...
                t_1 = solve_s_mpoly_mpoly(tmp_mr1, tmp_mr2, tmp_v1, tmp_v2,
                                          t_prev, EQ_1)
            if (tmp_cf2_prev == -1
                    or (tmp_cf2_prev - 1) % (2 * tmp_mr2.poly.n) == tmp_cf2
                    or (tmp_cf2_prev - 2) % (2 * tmp_mr2.poly.n) == tmp_cf2):
                t_2 = solve_s_mpoly_mpoly(tmp_mr1, tmp_mr2, tmp_v1, tmp_v2,
                                          t_prev, EQ_0)
            t_3 = solve_parallel_edges_mpoly_mpoly(tmp_mr1, tmp_mr2, tmp_v1,
                                                   tmp_v2, t_prev)
            t_4 = solve_parallel_edges_mpoly_mpoly(
                tmp_mr1, tmp_mr2, (tmp_v1 - 1) % tmp_mr1.poly.n, tmp_v2,
                t_prev)
            ...
            if t_1 <= 1 and t_1 < t_2 and t_1 < t_3 and t_1 < t_4:
                ...
                s21 = compute_s(
                    tmp_mr2.at_v(tmp_v2 + 1, t_4), tmp_mr1.at_v(
                        tmp_v1 - 1, t_4), tmp_mr1.at_v(tmp_v1, t_4))
                if 0 < s12 < 1:
                    tmp_cf1_prev = tmp_cf1
                    tmp_cf1 = (tmp_cf1 - 2) % (2 * tmp_mr1.poly.n)
                elif 0 < s21 < 1:
