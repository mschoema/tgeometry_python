...
class Poly():
  ...
  def random(xmin, ymin, xmax, ymax, n, center=True, digits=2):
    def gen_random_poly(xmin, ymin, xmax, ymax, n):
      ...
      b = points[i + 1]
      c = points[i + 2]
      ccw = (a[0] - b[0]) * (c[1] - b[1]) - (a[1] - b[1]) * (c[0] - b[0]) < 0
      for i in range(1, n - 2):
        ...
        if ccw and (a[0] - b[0]) * (c[1] - b[1]) - (a[1] - b[1]) * (
          ...
          convex = False
          break
        elif not ccw and (a[0] - b[0]) * (c[1] - b[1]) - (a[1] - b[1]) * (
            c[0] - b[0]) < 0:
          convex = False
          break
...
def main():
  x, y = 628999, 6334942
  width, height = 8000, 3000
  xmin, ymin = x - width / 2, y - height / 2
  xmax, ymax = x + width / 2, y + height / 2
  ns = [5, 10, 25, 50, 75]
  polys = []
  for n in ns:
    polys.append(Poly.random(xmin, ymin, xmax, ymax, n, False, 1))

  for poly in polys:
    points = ", ".join([f"{pt.x} {pt.y}" for pt in poly.pts])
